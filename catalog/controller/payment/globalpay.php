...
class ControllerPaymentGlobalpay extends Controller {
	...
	public function notify() {
		...
		if ($hash != $this->request->post['SHA1HASH']) {
			...
			$data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/checkout', '', true, $this->session->data['country_code'], $this->session->data['language_name']));
		} else {
			$this->load->model('checkout/order');
			...
			if ($this->request->post['RESULT'] == "00") {
				...
				$data['text_link'] = sprintf($this->language->get('text_link'), $this->url->link('checkout/success', '', true, $this->session->data['country_code'], $this->session->data['language_name']));
