...
class ControllerPaymentEway extends Controller {
	public function index() {
		...
		$request->RedirectUrl = $this->url->link('payment/eway/callback', '', true, $this->session->data['country_code'], $this->session->data['language_name']);
		...
		if (isset($lbl_error)) {
			...
			if ($this->config->get('eway_paymode') == 'iframe') {
				$data['callback'] = $this->url->link('payment/eway/callback', 'AccessCode=' . $result->AccessCode, true, $this->session->data['country_code'], $this->session->data['language_name']);
