...
class ControllerAccountReward extends Controller {
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/reward', '', true, $this->session->data['country_code'], $this->session->data['language_name']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/reward', '', true, $this->session->data['country_code'], $this->session->data['language_name'])
		);
		...
		foreach ($results as $result) {
			$data['rewards'][] = array(
				...
				'href'        => $this->url->link('account/order/info', 'order_id=' . $result['order_id'], true, $this->session->data['country_code'], $this->session->data['language_name'])
			);
		}
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('account/reward', 'page={page}', true, $this->session->data['country_code'], $this->session->data['language_name']);
