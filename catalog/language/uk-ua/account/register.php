<?php
//Переклад: Том'як Олег з любов'ю до Української мови та легкості Opencart
// Heading 
$_['heading_title_reg']        = 'Реєстрація';

// Text
$_['text_account']         = 'Особистий Кабінет';
$_['text_register']        = '*Поле, обовязкове для заповнення';
$_['text_account_already'] = 'Будь ласка, зареєструйтесь, щоб створити обліковий запис';
$_['text_your_details']    = 'Основні дані';
$_['text_your_address']    = 'Ваша адреса';
$_['text_newsletter']      = 'Підписка на новини';
$_['text_your_password']   = 'Ваш пароль';
$_['text_agree']           = 'Мною прочитані, і я погоджуюсь з документом <a href="%s" class="agree"> <b>%s </b> </a>';

// Entry
$_['entry_customer_group'] = 'Група покупців';
$_['entry_firstname']      = 'Ім’я*';
$_['entry_lastname']       = 'Прізвище*';
$_['entry_email']          = 'Електронна пошта*';
$_['entry_telephone']      = 'Телефон*';
$_['entry_fax']            = 'Факс';
$_['entry_company']        = 'Компанія';
$_['entry_address_1']      = 'Адреса';
$_['entry_address_2']      = 'Адреса (додатково)';
$_['entry_postcode']       = 'Індекс';
$_['entry_city']           = 'Місто';
$_['entry_country']        = 'Країна';
$_['entry_zone']           = 'Регіон / Область';
$_['entry_newsletter']     = 'Підписатися на новини';
$_['entry_password']       = 'Пароль (буквы и цифры)*';
$_['entry_confirm']        = 'Підтвердити пароль*';

// Error
$_['error_exists']         = 'Цей E-Mail вже зареєстрований!';
$_['error_firstname']      = 'Ім’я має містити від 1 до 32 символів!';
$_['error_lastname']       = 'Прізвище має містити від 1 до 32 символів!';
$_['error_email']          = 'E-Mail введено невірно!';
$_['error_telephone']      = 'У номеру телефона має бути від 3 до 32 цифр!';
$_['error_address_1']      = 'Адреса має містити від 3 до 128 символів!';
$_['error_city']           = 'Назва міста має містити від 2 до 128 символів!';
$_['error_postcode']       = 'В індексі має бути від 2 до 10 символів!';
$_['error_country']        = 'Оберіть країну!';
$_['error_zone']           = 'Оберіть регіон / область!';
$_['error_custom_field']   = '%s необхідний!';
$_['error_password']       = 'У паролі має бути від 4 до 20 символів!';
$_['error_confirm']        = 'Паролі не співпадають!';
$_['error_agree']          = 'Для реєстрації Ви повинні погодитися з документом %s!';