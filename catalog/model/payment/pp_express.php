...
class ModelPaymentPPExpress extends Model {
	...
	public function paymentRequestInfo() {
		...
		foreach ($this->cart->getProducts() as $item) {
			...
			$data['L_PAYMENTREQUEST_0_ITEMURL' . $i] = $this->url->link('product/product', 'product_id=' . $item['product_id'], false, $this->session->data['country_code'], $this->session->data['language_name']);
