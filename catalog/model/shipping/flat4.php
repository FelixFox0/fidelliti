...
//            var_dump($this->currency->format($this->tax->calculate($this->config->get('flat4_cost'), $this->config->get('flat4_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency']));
//            var_dump($this->session->data['country_code']);
		...
		if ($status) {
			...
			$quote_data['flat4'] = array(
				...
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('flat4_cost'), $this->config->get('flat4_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
