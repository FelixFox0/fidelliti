<?php
//Переклад: Том'як Олег з любов'ю до Української мови та легкості Opencart
// Heading
$_['heading_title'] 		= 'eBay';
$_['text_openbay'] 		= 'OpenBay Pro';
$_['text_dashboard'] 		= 'Панель управління eBay';

// Text
$_['text_success'] 		= 'Ви зберегли ваші зміни в eBay розширенні';
$_['text_heading_settings'] 	= 'Налаштування';
$_['text_heading_sync'] 	= 'Синхронізація';
$_['text_heading_subscription'] = 'Змінити тарифний план';
$_['text_heading_usage'] 	= 'Використання';
$_['text_heading_links'] 	= 'Посилання на предмет';
$_['text_heading_item_import'] 	= 'Імпорт елементів';
$_['text_heading_order_import'] = 'Імпорт замовлень';
$_['text_heading_adds'] 	= 'Встановлені доповнення';
$_['text_heading_summary'] 	= 'eBay резюме';
$_['text_heading_profile'] 	= 'Профілі';
$_['text_heading_template'] 	= 'Шаблони';
$_['text_heading_ebayacc'] 	= 'Аккаунт eBay';
$_['text_heading_register'] 	= 'Зареєструйтеся тут';

// Error
$_['error_category_nosuggestions'] = 'Не вдалося завантажити будь-які пропоновані категорії';
$_['error_loading_catalog'] 	= 'У каталозі eBay, не знайдено';
$_['error_generic_fail'] 	= 'Виникла невідома помилка!';